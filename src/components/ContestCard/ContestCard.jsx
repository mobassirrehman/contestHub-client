      ...
      <div className="card bg-base-100 shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-base-200 h-full">
        <figure className="relative h-48 overflow-hidden">
          ...
          <div className="absolute bottom-3 left-3">
            <span className="bg-blue-500/30 text-primary-content text-xs font-medium px-3 py-1 rounded-full capitalize">
            ...
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                {[...Array(Math.min(3, participantsCount || 1))].map((_, i) => (
                  <div
                    key={i}
                    className="w-7 h-7 rounded-full bg-gradient-to-r from-cyan-400/60 to-[#9177C7]/70 flex items-center justify-center text-xs text-white border-2 border-base-100"
                  >
                    {i + 1}
                  </div>
                ))}
          ...
          <div className="card-actions mt-4">
            <button
              ...
              className={`btn w-full ${
                isEnded
                  ? "btn-disabled"
                  : "btn btn-gradient-primary hover:scale-[1.02]"
